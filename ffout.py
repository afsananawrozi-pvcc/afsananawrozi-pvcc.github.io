...
def create_output_file():
    ...
    f.write(line)
    f.write('\n************* FRESH FODDS MARKET ***************')
    f.write('\n------------- WEEKLY PAYROLL REPORT ____________')
    ...
    for i in range (num_emps):
        ...
        data1 = "\n" + emp[i] + " " + job[i] + format(gross_pay[i], currency) + format(fed_tax[i], currency) + format(state_tax[i], currency) + format(ret[i], currency) + format(soc_sec[i], currency) + format(medicare[i], currency) + format(net_pay[i], currency) 
        f.write(data1)
