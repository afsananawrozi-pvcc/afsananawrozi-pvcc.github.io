# Name: Afsana Nawrozi
# Prog Purpose: This program creates a payroll report for a Fresh Food Market.
...
def perform_calculations():
    ...
    #calculate deductions
        ...
        net = pay - fed - state - ss - med - retirement
        total_gross += pay
        total_net += net
...
def display_results():
    ...
    print(line)
    print('************* FRESH FODDS MARKET ***************')
    print('------------- WEEKLY PAYROLL REPORT ____________')
    print(tab + str(datetime.datetime.now()))
    print(line)
    titles1 = "Emp Name" + tab + "Code" + tab + "Gross" + tab
    titles2 = "Fed Inc Tax" + tab + "State Inc Tax" + tab + "Soc Sec" + tab + "Medicare" + tab + "Net" +tab
    print(titles1 + titles2)

    for i in range (num_emps):
      
        data1 = emp[i] + " " + job[i] + format(gross_pay[i], currency) + format(fed_tax[i], currency) + format(state_tax[i], currency) + format(ret[i], currency) + format(soc_sec[i], currency) + format(medicare[i], currency) + format(net_pay[i], currency) 
        print(data1)
